...
func GetIntersectionNode(A, B *listNode) *listNode {
	sizeA, sizeB := getSize(A), getSize(B)
	curA, curB := A, B
	diff := sizeA - sizeB
	//if lists are of different length, make the starting point of the lists be at equal distance from intersection node
	if diff > 0 {
		for i := 0; i < diff; i++ {
			curA = curA.next
		}
