...
*/
func GetSweetestCandies(A []int, B []int, C []int, D int) int {
	N := len(A)
	nibbles := D
	memo := make([][]int, N+1)
	for i := range memo {
		memo[i] = make([]int, nibbles+1)
	}
	...
	for i := 1; i <= N; i++ {
		for j := 1; j <= nibbles; j++ {
			...
			//if the money in hand is greater than the cost
			//include the element

			//sweetness: current sweetness + previous sweetness
			if j >= C[i-1] {
				memo[i][j] = max(memo[i][j], A[i-1]*B[i-1]+memo[i][j-C[i-1]])
			}
		}
	}
	return memo[N][nibbles]
}
