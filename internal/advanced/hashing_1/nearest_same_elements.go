...
*/
func GetNearestDistance(A []int) int {
	var ans int = 1e5
	var diff int
	lastVisitedMap := make(map[int]int)
	for i := range A {
		if idx, ok := lastVisitedMap[A[i]]; ok {
			diff = i - idx
			//if the same elements are closer than the last pair
			if diff < ans {
				ans = diff
			}
		}
		lastVisitedMap[A[i]] = i
	}
	if ans == 1e5 {
		return -1
	}
