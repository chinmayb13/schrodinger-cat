...
*/
func DoKDistanceSorting(A []int, B int) []int {
	mnh := make(minIntHeap, 0)
	heap.Init(&mnh)
	//make minHeap for the window size
	for i := 0; i <= B; i++ {
		heap.Push(&mnh, A[i])
	}
	//add the min of heap in the 0th index
	A[0] = heap.Pop(&mnh).(int)
	k := 1
	//move the window and iteratively update the array
	for i := B + 1; i < len(A); i++ {
		heap.Push(&mnh, A[i])
		A[k] = heap.Pop(&mnh).(int)
		k++
	}
	for k < len(A) {
		A[k] = heap.Pop(&mnh).(int)
