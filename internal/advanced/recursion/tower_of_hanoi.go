...
func TowerOfHanoi(A int) [][]int {
	var ans [][]int
	arrangeDisks(A, A, 1, 3, 2, &ans)
	return ans
}

func arrangeDisks(val, stackSize, source, target, auxilliary int, steps *[][]int) {
	if stackSize == 1 {
		*steps = append(*steps, []int{val, source, target})
		return
	}
	arrangeDisks(val-1, stackSize-1, source, auxilliary, target, steps)
	arrangeDisks(val, 1, source, target, auxilliary, steps)
	arrangeDisks(val-1, stackSize-1, auxilliary, target, source, steps)
