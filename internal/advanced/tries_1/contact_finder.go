...
*/
func GetContactPrefixList(A []int, B []string) []int {
	root := newTrieNode()
	var ans []int
	for i := range A {
		if A[i] == 0 {
			...
			ans = append(ans, getCount(root, B[i]))
		}
	}
	return ans

}
func insert(root *trieNode, inp string) *trieNode {
	cur := root
	for i := range inp {
		if _, ok := cur.child[inp[i]]; !ok {
			cur.child[inp[i]] = newTrieNode()
		}
		cur = cur.child[inp[i]]
		cur.count++
	}
...
func getCount(root *trieNode, inp string) int {
	cur := root
	for i := range inp {
		if _, ok := cur.child[inp[i]]; !ok {
			return 0
