...
func GetBatchCount(A int, B []int, C [][]int, D int) int {
	...
	for i := range C {
		adjList[C[i][0]] = append(adjList[C[i][0]], C[i][1])
		adjList[C[i][1]] = append(adjList[C[i][1]], C[i][0])
	}
	for i := 1; i <= A; i++ {
		//for each connected component check the strength
		if !visited[i] {
			strength := doDFS(i, B, adjList, visited)
			if strength >= D {
				ans++
			}
		}
...
func doDFS(vertice int, strength []int, adjList [][]int, visited []bool) int {
	visited[vertice] = true
	ans := 0
	for i := range adjList[vertice] {
		v := adjList[vertice][i]
		if !visited[v] {
			ans += doDFS(v, strength, adjList, visited)
		}
	}
	return ans + strength[vertice-1]
}
