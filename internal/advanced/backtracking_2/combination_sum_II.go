...
func CombinationSumII(A []int, B int) [][]int {
	...
	//start with 0 index
	getCombinationsII(B, 0, cur, A, &ans)
	return ans
}

func getCombinationsII(sum int, idx int, cur []int, inp []int, ans *[][]int) {
	//base successful condition
	if sum == 0 {
		*ans = append(*ans, copyIntArr(cur))
		return
	}

	//base negative condition
	if sum < 0 || idx == len(inp) {
		return
	}
	...
	for i := idx; i < len(inp); i++ {
		...
		//jump to the next idx
		getCombinationsII(sum-inp[i], i+1, cur, inp, ans)
		cur = cur[:len(cur)-1]

		//jump to the next idx with unique element
