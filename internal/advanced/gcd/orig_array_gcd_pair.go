...
*/
func GetOrigArray(A []int) []int {
	var originalArr []int
	origArrLength := int(math.Sqrt(float64(len(A))))
	for i := 0; i < len(A); i += origArrLength {
		originalArr = append(originalArr, getMax(i, i+origArrLength, A))
	}
	return originalArr
}

func getMax(start, end int, inp []int) int {
	max := 0
	for i := start; i < end; i++ {
		if inp[i] > max {
			max = inp[i]
